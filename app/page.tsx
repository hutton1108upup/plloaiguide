...
function FeatureIcon({ icon }: { icon: (typeof features)[number]['icon'] }) {
  const paths = {
    'text-to-video': <><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m10 9 5 3-5 3V9Z" /></>,
    'image-to-video': <><rect x="3" y="5" width="14" height="14" rx="2" /><path d="m6 16 3.5-4 2.5 2 2-2.5 3 3.5M16 8h5m-2-2 2 2-2 2" /></>,
    'fast-generation': <path d="m13 2-8 12h7l-1 8 8-12h-7l1-8Z" />,
    'style-consistency': <><rect x="3" y="4" width="13" height="13" rx="2" /><rect x="8" y="9" width="13" height="11" rx="2" /><path d="m11 14 2-2 5 5" /></>,
  };

  return (
    <svg aria-hidden="true" viewBox="0 0 24 24">{paths[icon]}</svg>
  );
}
...
function AffiliateButton({ children, path = '', variant = 'primary' }: {
  ...
  return (
    <a className={`cta-button cta-button--${variant}`} href={affiliateUrl(path)} target="_blank" rel={affiliateRel}>
      <span>{children}</span><ArrowIcon /><ExternalLinkHint />
    </a>
  );
}
...
function ModelRows({ items, startIndex = 0 }: {
  ...
  return items.map((model, index) => (
    <tr key={model.family}>
      <td data-label="Model family">
        <span className="model-number">{String(index + startIndex + 1).padStart(2, '0')}</span>
        <a className="model-family-link" href={affiliateUrl(model.listed[0].path)} target="_blank" rel={affiliateRel}>
          {model.family}<span aria-hidden="true">↗</span><ExternalLinkHint />
        </a>
      </td>
      <td data-label="Models currently listed">
        <div className="model-link-list">
          {model.listed.map((item) => (
            <a className="model-chip" href={affiliateUrl(item.path)} key={`${model.family}-${item.name}`} target="_blank" rel={affiliateRel}>
              {item.name}<ExternalLinkHint />
            </a>
          ))}
        </div>
      </td>
    ...
    <>
      ...
      <main id="main-content">
        <section className="hero" id="top">
          ...
          <div className="shell hero-inner">
            ...
            <div className="hero-actions reveal reveal--four">
              ...
              <span className="cta-note">Free credits available for new users. Model limits vary.</span>
            </div>
        ...
        <section className="section section--intro">
          <div className="shell intro-grid">
            ...
            <div className="intro-copy">
              <p>Many creators searching for “polo ai” are actually looking for Pollo.ai. It is a common typo for a multi-model AI video generation workspace.</p>
        ...
        <section className="section section--models" id="models">
          <div className="shell">
            <div className="models-heading">
              <div className="section-heading"><span className="eyebrow">Choose your engine</span><h2>Featured AI Video Models on Pollo AI</h2></div>
              <p>This curated list highlights widely used model families available in Pollo AI. Choose based on your workflow, output style, and the options currently available in your plan.</p>
            </div>
            <div className="model-table-wrap">
              <table className="model-table">
                ...
                <tbody><ModelRows items={featuredModels} /></tbody>
              </table>
            </div>
            <details className="model-catalog">
              <summary aria-label={`View all ${models.length} models`}>
                <span><strong>View all {models.length} models</strong><small>{remainingModels.length} more model families</small></span>
                <i aria-hidden="true" />
              </summary>
              <div className="model-table-wrap model-table-wrap--expanded">
                <table className="model-table">
                  <thead><tr><th scope="col">Model family</th><th scope="col">Models currently listed</th><th scope="col">Developer</th></tr></thead>
                  <tbody><ModelRows items={remainingModels} startIndex={featuredModels.length} /></tbody>
                </table>
              </div>
            </details>
            <p className="source-note">Featured list, not an exhaustive catalogue. Model availability was checked against Pollo AI&apos;s official model pages in {SITE.modelVerifiedAt}. Versions, plan access, and credit requirements may change.{' '}<a href={affiliateUrl('/m')} target="_blank" rel={affiliateRel}>Compare Current Models<span aria-hidden="true"> ↗</span><ExternalLinkHint /></a></p>
          </div>
        </section>
        ...
        <section className="section section--how" id="how-to">
          <div className="shell how-grid">
            <div className="how-intro">
              ...
              <AffiliateButton path="/text-to-video">Try Text-to-Video</AffiliateButton>
            </div>
        ...
        <section className="final-cta">
          ...
          <div className="shell final-cta-inner"><span className="eyebrow">Your next video starts here</span><h2>Turn your idea into motion.</h2><p>Explore Pollo.ai&apos;s models and start creating from text or images today.</p><AffiliateButton variant="secondary">Start with Free Credits</AffiliateButton></div>
        </section>
      </main>
      ...
      <footer className="site-footer">
        <div className="shell footer-grid">
          ...
          <div className="footer-links"><Link href="/privacy">Privacy Policy</Link><Link href="/terms">Terms</Link><a href="#models">Models</a><a href="#how-to">How it works</a><a href="#faq">FAQ</a></div>
        </div>
